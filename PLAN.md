...
**処理フロー（音声）**:
1. ユーザーがMP3をアップロード
2. バックエンドがGoogle Cloud Storageに一時保存
3. Gemini File APIにアップロード
4. Geminiが音声を直接解析し、議事録を生成
5. 一時ファイルを削除

### Cloud Run構成
- **タイムアウト**: 最大60分（2時間音声の処理に対応）
- **メモリ**: 1GB（大きなMP3ファイルのハンドリング用）
- **CPU**: 2 vCPU
- **最大リクエストサイズ**: Cloud Storageへの署名付きURL直接アップロードで回避
- **同時実行数**: 1（LLM処理は重いため）
